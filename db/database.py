        ...
        if db_manager:
            try:
                ...
                if db_profile:
                    ...
                    logger.info(f"✅ Loaded user profile from DB for session: {session_id}")
                else:
                    logger.info(f"ℹ️ No existing profile found, using default for session: {session_id}")
            ...
            logger.warning("⚠️ Database manager not available, using memory-only session")
    ...
    try:
        ...
        if db_manager:
            ...
            if update_success:
                ...
                logger.warning(f"⚠️ Failed to update user info in database: {session_id}")
            ...
            if profile_success:
                ...
                logger.warning(f"⚠️ Failed to update user profile in database: {session_id}")
    ...
    try:
        ...
        try:
            ...
            if not existing_user:
                user_created = db_manager.create_user_record(session_id)
                if user_created:
                    logger.info(f"✅ Created user record for profile save: {session_id}")
                else:
                    logger.warning(f"⚠️ Failed to create user record, but continuing: {session_id}")
        except Exception as e:
        ...
        if user_profile.get('name') or user_profile.get('phone') or user_profile.get('address'):
            ...
            try:
                update_success = db_manager.update_user_info(session_id, user_data)
                if update_success:
                    logger.info(f"✅ Updated user info in users table for session: {session_id}")
                else:
                    logger.warning(f"⚠️ Failed to update user info for session: {session_id}")
            except Exception as e:
        ...
        try:
            ...
            if result:
                logger.info(f"✅ User profile saved successfully for session: {session_id}")
                return True
            else:
                logger.warning(f"⚠️ Failed to save user profile for session: {session_id}")
                return False
        except Exception as e:
    ...
    try:
        if db_manager:
            ...
            if not existing_user:
                user_created = db_manager.create_user_record(session_id)
                if user_created:
                    logger.info(f"✅ Created user record before saving chat session: {session_id}")
                else:
                    logger.error(f"❌ Failed to create user record for chat session: {session_id}")
                    return
            ...
            logger.warning("⚠️ Database manager not available, skipping chat session save")
...
def cleanup_old_sessions(max_age_hours: int = 24) -> int:
    ...
    try:
        ...
        logger.info(f"✅ Cleanup completed, removed {len(sessions_to_remove)} old sessions")
...
def get_session_statistics() -> dict:
    ...
    try:
        ...
        sessions_with_profiles = sum(1 for s in user_sessions.values() if s.get("profile", {}).get("name"))
...
def create_order_integrated(session_id: str, product_info: dict) -> tuple:
    ...
    try:
        ...
        from services.order import create_order_with_better_product_handling_v2
...
def batch_sync_sessions() -> dict:
    ...
    logger.info(f"✅ Batch sync completed: {results['synced_successfully']}/{results['total_sessions']} successful")
    return results