...
from models import (
    ChatMessage, ChatResponse, UserProfile, Product, ProductSearchRequest,
    OrderRequest, OrderResponse, OrderInfo, OrderStatusUpdate, ContactInfo, SystemStats
)
 
from db.sql_api import DatabaseManager
from services.create_images import debug_image_pipeline
from services.order import test_improved_matching
from startup import initializer
# Import DatabaseManager from the correct path
from services.chat_logic import debug_process_chat_message, process_chat_message, test_image_functionality
# ===== GLOBAL VARIABLES =====
load_dotenv()
# Google API Key
...
async def chat(request: ChatMessage):
    ...
    try:
        ...
        try:
            ...
            for field in required_fields:
                if field not in result:
                    result[field] = getattr(request, field, '') if hasattr(request, field) else ''
                        
            # Ensure response is not empty
            if not result.get('response'):
                result['response'] = "Xin lỗi, tôi không thể xử lý yêu cầu này lúc này. Vui lòng thử lại."
            ...
            print(f"📊 Image data: {result.get('images', [])[:1]}")  # Log first image for debug
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Lỗi server không mong đợi: {str(e)}")
...
async def create_order(request: OrderRequest):
    ...
    try:
        ...
        if total_amount == 0:
            raise HTTPException(status_code=400, detail="Không tìm thấy sản phẩm hợp lệ")
...
async def get_order(order_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi lấy thông tin đơn hàng: {str(e)}")
...
async def get_user_orders(user_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi lấy danh sách đơn hàng: {str(e)}")
...
async def update_order_status(order_id: str, status: str):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail="Không tìm thấy đơn hàng hoặc không thể cập nhật")
...
async def get_vector_store_status_endpoint():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi kiểm tra vector store: {str(e)}")
...
def trigger_test_search():
    try:
        ...
        return {"status": "success", "message": "test_search_function() đã được thực thi"}
    except Exception as e:
