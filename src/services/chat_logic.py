...
from services.create_images import (
    create_image_gallery_html,
    detect_image_request,
    extract_product_name_from_image_request,
    get_product_images,
    handle_image_request_with_display,
)
...
def process_chat_message(message: str, session_id: str):
    ...
    if detect_image_request(message):
        ...
        try:
            response_text, images, html_content = handle_image_request_with_display(
...
Mình sẽ cố gắng khắc phục ngay! 😊"""
    ...
    if purchase_intent and not user_profile.get("collecting_order_info"):
        ...
        user_profile["selected_product"] = extract_product_from_message_improved(
            message
        )
...
Bạn gõ số hoặc tên phương thức giúp mình nhé! 🙏"""
        ...
        if response is None:
            ...
            try:
                ...
                order_id, total_amount, product_details = (
                    create_order_with_better_product_handling_v2(
                        session_data_for_order,
                        user_profile,
                    )
                ...
                if order_id == "ERROR":
                    ...
                    payment_method = get_payment_method_display(
                        user_profile.get("payment_method")
                    )
...
Bạn có muốn xem thêm sản phẩm nào khác không? 😊✨"""
    ...
    try:
        ...
        recommended_products = get_relevant_products_enhanced(
            message, user_profile, limit=5
        )
        ...
        print(
            f"📊 Product recommendation result: {len(recommended_products)} products, {len(product_images)} images"
        )
...
Mình sẽ hỗ trợ bạn ngay! 😊"""
    ...
    return {
        ...
        "total_products_found": (
            len(recommended_products) if "recommended_products" in locals() else 0
        ),
    }
...
def cleanup_and_optimize() -> dict:
    ...
    try:
        ...
        logger.info(
            f"✅ Optimization completed: {cleaned_sessions} sessions cleaned, {sync_result['synced_successfully']} synced"
        )
...
def health_check() -> dict:
    ...
    try:
        ...
        # Check vector store
        vector_status = (
            hasattr(globals, "vectorstore") and globals.vectorstore is not None
        )
    ...
    if is_image_request:
        ...
        if product_name:
            try:
                ...
                for i, img in enumerate(images):
                    print(
                        f"🐛 DEBUG:   {i+1}. {img.get('product_name')} - {img.get('image_url')}"
                    )
            except Exception as e:
